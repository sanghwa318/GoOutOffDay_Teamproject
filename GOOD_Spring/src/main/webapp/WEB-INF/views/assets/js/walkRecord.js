...
function startRecord() {
    if (navigator.geolocation) { // GPS를 지원하면
        watchId = navigator.geolocation.watchPosition(function(position) {
            ...
            console.log("위도: " + wat_latitude + ", 경도: " + wat_longitude + ", 정확도: " + wat_accuracy + ", 타임스탬프: " + wat_timestamp)
            //  이 자리에서 ajax로 db에 올리는 요청을 보내야하나..?
            //if (wat_accuracy < 300) {
            $.ajax({
                url: getContextPath() + '/walkPage/walk_record.do',
                type: 'post',
                dataType: 'json',
                data: { wat_latitude, wat_longitude, wat_timestamp },
