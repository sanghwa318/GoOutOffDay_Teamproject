			...
			<div class="row cas_list">
				...
				<div class="orderBy_tag">
					...
					<a href='../casPage/cas_themeList.jsp?cas=<%=cas%>'
						class='cas_category' data-filter='전체'>전체</a> <a
						href='../casPage/cas_themeList_order.jsp?cas=<%=cas%>&order=교육체험'
						id='orderby'
						class='cas_category <%if (order.equals("교육체험")) {%>active<%}%>'
						data-filter='교육체험'>교육체험</a> <a
						href='../casPage/cas_themeList_order.jsp?cas=<%=cas%>&order=농장체험'
						id='orderby'
						class='cas_category <%if (order.equals("농장체험")) {%>active<%}%>'
						data-filter='농장체험'>농장체험</a> <a
						href='../casPage/cas_themeList_order.jsp?cas=<%=cas%>&order=문화행사'
						id='orderby'
						class='cas_category <%if (order.equals("문화행사")) {%>active<%}%>'
						data-filter='문화행사'>문화행사</a> <a
						href='../casPage/cas_themeList_order.jsp?cas=<%=cas%>&order=전시관람'
						id='orderby'
						class='cas_category <%if (order.equals("전시관람")) {%>active<%}%>'
						data-filter='전시/관람'>전시/관람</a> <a
						href='../casPage/cas_themeList_order.jsp?cas=<%=cas%>&order=기타'
						id='orderby'
						class='cas_category <%if (order.equals("기타")) {%>active<%}%>'
						data-filter='기타'>기타</a>
					<%
					}
					%>
					...
					<a href='../casPage/cas_themeList.jsp?cas=<%=cas%>'
						...
						id='orderby'
						class='cas_category <%if (order.equals("대중음악")) {%>active<%}%>'
						data-filter='대중음악'>대중음악</a> <a
						href='../casPage/cas_themeList_order.jsp?cas=<%=cas%>&order=기획전시'
						id='orderby'
						class='cas_category <%if (order.equals("기획전시")) {%>active<%}%>'
						data-filter='기획전시'>기획전시</a> <a
						href='../casPage/cas_themeList_order.jsp?cas=<%=cas%>&order=오페라'
						id='orderby'
						class='cas_category <%if (order.equals("오페라")) {%>active<%}%>'
						data-filter='오페라'>오페라</a> <a
						href='../casPage/cas_themeList_order.jsp?cas=<%=cas%>&order=연극'
						id='orderby'
						class='cas_category <%if (order.equals("연극")) {%>active<%}%>'
						data-filter='연극'>연극</a> <a
						href='../casPage/cas_themeList_order.jsp?cas=<%=cas%>&order=기타'
						id='orderby'
						class='cas_category <%if (order.equals("기타")) {%>active<%}%>'
						data-filter='기타'>기타</a>
					...
					<a href='../casPage/cas_themeList.jsp?cas=<%=cas%>'
						...
						id='orderby'
						class='cas_category <%if (order.equals("공예취미")) {%>active<%}%>'
						data-filter='공예/취미'>공예/취미</a> <a
						href='../casPage/cas_themeList_order.jsp?cas=<%=cas%>&order=교양어학'
						id='orderby'
						class='cas_category <%if (order.equals("교양어학")) {%>active<%}%>'
						data-filter='교양/어학'>교양/어학</a> <a
						href='../casPage/cas_themeList_order.jsp?cas=<%=cas%>&order=스포츠'
						id='orderby'
						class='cas_category <%if (order.equals("스포츠")) {%>active<%}%>'
						data-filter='스포츠'>스포츠</a> <a
						href='../casPage/cas_themeList_order.jsp?cas=<%=cas%>&order=전문자격증'
						id='orderby'
						class='cas_category <%if (order.equals("연극")) {%>active<%}%>'
						data-filter='전문/자격증'>전문/자격증</a> <a
						href='../casPage/cas_themeList_order.jsp?cas=<%=cas%>&order=기타'
						id='orderby'
						class='cas_category <%if (order.equals("기타")) {%>active<%}%>'
						data-filter='기타'>기타</a>
					...
					<a href='../casPage/cas_themeList.jsp?cas=<%=cas%>'
						...
						id='orderby'
						class='cas_category <%if (order.equals("체육관")) {%>active<%}%>'
						data-filter='체육관'>체육관</a> <a
						href='../casPage/cas_themeList_order.jsp?cas=<%=cas%>&order=축구장'
						id='orderby'
						class='cas_category <%if (order.equals("축구장")) {%>active<%}%>'
						data-filter='축구장'>축구장</a> <a
						href='../casPage/cas_themeList_order.jsp?cas=<%=cas%>&order=풋살장'
						id='orderby'
						class='cas_category <%if (order.equals("풋살장")) {%>active<%}%>'
						data-filter='풋살장'>풋살장</a> <a
						href='../casPage/cas_themeList_order.jsp?cas=<%=cas%>&order=다목적경기장'
						id='orderby'
						class='cas_category <%if (order.equals("다목적경기장")) {%>active<%}%>'
						data-filter='다목적경기장'>다목적경기장</a> <a
						href='../casPage/cas_themeList_order.jsp?cas=<%=cas%>&order=기타'
						id='orderby'
						class='cas_category <%if (order.equals("기타")) {%>active<%}%>'
						data-filter='기타'>기타</a>
					...
					<div class="col-xs-6 col-sm-4 col-md-3 casItem">
						<div class="thumbnail item">
							...
							<div class="caption clearfix">
								<p>
									<%=result%><span class="middel_dot"></span><%=order%><span class="heart pull-right"><i
